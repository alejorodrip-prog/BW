...
- `Kubota L2502 Compact Tractor - optimized/` (100 fotos, ~2 MB) y `Kubota ZD1211 Zero-Turn Mower - optimized/` (100 fotos, ~2 MB): las dos primeras secuencias 360° reales para Inventory Highlights, ya listas para usar. Todavía faltan 1–2 productos más para completar las 3–4 de esa sección.
...
**Assets nuevos (2026-09-08):** ya llegaron las primeras dos secuencias 360° reales para Inventory Highlights (Kubota L2502 Compact Tractor y Kubota ZD1211 Zero-Turn Mower), verificadas cuadro por cuadro (100 frames únicos cada una, sin duplicados) y optimizadas de ~75-85MB en PNG a ~2MB cada una en JPEG 640×360. Reemplazan el placeholder "coming soon" para esos dos productos; ver PROVIDED ASSETS y ASSET INTEGRATION abajo.
...
2. Inventory Highlights. Two real 360° sequences are attached and ready to use:
   `Kubota L2502 Compact Tractor - optimized/` and
   `Kubota ZD1211 Zero-Turn Mower - optimized/`, 100 frames each, numbered 001.jpg to
   100.jpg, 640×360, about 2MB total per product. Build these two highlight cards
   directly against these frames (see ASSET INTEGRATION for the exact algorithm),
   using the matching CONFIRMED INVENTORY entries: the L2502 under TRACTORS →
   Compact → L Series, and the ZD1211 under MOWERS → Diesel → ZD Series. Reserve
   space for 1–2 more highlighted products (3–4 total, not more), picked from
   CONFIRMED INVENTORY groups not already covered by these two (e.g. a Utility
   Vehicle or Attachment), so the highlights span at least two, ideally more, of the
   four groups. Build those remaining card(s) with the same 360°-viewer anatomy
   (drag/swipe, auto-play once, see ASSET INTEGRATION) so they're ready to receive
   real frames, but show a "360° view coming soon" placeholder state until that
   footage exists. Don't name or imply a specific real model on any placeholder card
   until we confirm which unit is being shot.
...
(mechanism unchanged). The hero sequence is real and attached; two of the
highlight-card sequences (L2502, ZD1211) are now real and attached too; any
remaining highlight card(s) stay placeholder, pending production. See PROVIDED
ASSETS.
...
- Real sequences: `Kubota L2502 Compact Tractor - optimized/001.jpg` through
  `100.jpg`, and `Kubota ZD1211 Zero-Turn Mower - optimized/001.jpg` through
  `100.jpg`, 100 frames each, 640×360 (see PROVIDED ASSETS). Any remaining
  placeholder highlight card(s) should be built to the same 100-frame, 640×360 spec
  so they're ready to receive real footage without a rebuild. Preload all frames per
  ...
  until ready.
- Interaction: on pointerdown, track horizontal pointer movement. Map every ~4–6px
  of horizontal drag to one frame step (tune to taste; more frames than the earlier
  ~60-frame estimate, so a slightly smaller step keeps the rotation feeling
  proportional). Wrap frame index modulo the frame count in both directions so the
  rotation feels continuous. Support both mouse drag and touch swipe (pointer events
  cover both).
